...
def extract_chiral_conformers(args):
    drugs_file = os.path.join(args.data_dir, args.data_file)
    save_file = f"geom_drugs_chiral_{'no_h_' if args.remove_h else ''}{args.conformations}"
    smiles_list_file = 'geom_drugs_chiral_smiles.txt'
    number_atoms_file = f"geom_drugs_n_chiral_{'no_h_' if args.remove_h else ''}{args.conformations}"
    mapping_file = f"geom_drugs_conformer_to_molecule_chiral_{'no_h_' if args.remove_h else ''}{args.conformations}.npy"
    ...
    for i, drugs_1k in enumerate(unpacker):
        ...
        for smiles, all_info in drugs_1k.items():
            if not is_chiral_from_smiles(smiles):
                continue
            ...
            for id in lowest_energies:
                ...
                conformer_to_molecule.append(len(all_smiles) - 1)
                mol_id += 1

    print("Total number of chiral conformers saved", mol_id)
    all_number_atoms = np.array(all_number_atoms)
    dataset = np.vstack(dataset_conformers)
    conformer_to_molecule = np.array(conformer_to_molecule)
    
    print("Total number of atoms in the chiral dataset", dataset.shape[0])
    print("Average number of atoms per chiral molecule", dataset.shape[0] / mol_id)
    ...
    np.save(os.path.join(args.data_dir, mapping_file), conformer_to_molecule)
...
if __name__ == '__main__':
    ...
    extract_chiral_conformers(args)
